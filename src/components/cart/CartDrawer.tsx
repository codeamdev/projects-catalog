...
export function CartDrawer({ tenantName, tenantSubdomain, whatsappNumber, discountCode, discountCodePercent }: Props) {
  ...
  async function handleSendOrder(e: React.FormEvent) {
    ...
    // Formato tabular con tabs → pega directo en Excel
    // Columnas: Producto | Cant. | P. Unitario | Descuento | P. Final | Total ítem
    const T = "\t";
    const header = `Producto${T}Cant.${T}P. Unitario${T}Descuento${T}P. Final${T}Total`;
    const itemRows = items.map((i) => {
      const qty = i.quantity;
      if (!i.price) return `${i.title}${T}${qty}${T}${T}${T}${T}`;
      if (i.discountPercent && i.originalPrice) {
        return `${i.title}${T}${qty}${T}${fmtCOP(i.originalPrice)}${T}-${i.discountPercent}%${T}${fmtCOP(i.price)}${T}${fmtCOP(i.price * qty)}`;
      }
      return `${i.title}${T}${qty}${T}${fmtCOP(i.price)}${T}${T}${fmtCOP(i.price)}${T}${fmtCOP(i.price * qty)}`;
    });
    ...
    const parts = [
      ...
      couponApplied && discountCodePercent
        ? `${T}${T}${T}Subtotal${T}${T}${fmtCOP(subtotal)}`
        : null,
      couponApplied && discountCodePercent
        ? `${T}${T}${T}Desc. ${couponInput.toUpperCase()} -${discountCodePercent}%${T}${T}-${fmtCOP(couponDiscount)}`
        : null,
      `${T}${T}${T}TOTAL${T}${T}${fmtCOP(finalTotal)}`,
      "",
      "---",
      `Nombre: ${name}`,
    ...
    const url = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(parts)}`;
    if (waWindow) {
